...
class Flame(object):
    ...
    def burn(self, spark_cells=[0, 1, 2, 5, 7, 9, 10], threshold=0.1, explosion_heat=[1000, 1200]):
    ...
    def calculate_temperature(self, span=2):
        for i in range(self.size//2):
            ...
            self.cells[i] = max(0, new_temp - (self.cooling+i*1.2)) if not np.isnan(new_temp) else 0.0
            ...
            self.cells[inverted_i] = max(0, new_temp - (self.cooling+i*1.2)) if not np.isnan(new_temp) else 0.0
