...
class Flame(object):
    ...
    def calculate_temperature(self, span=2):
        for i in range(self.size//2):
            ...
            self.cells[i] = max(0, new_temp - (self.cooling+i)) if not np.isnan(new_temp) else 0.0
            ...
            self.cells[inverted_i] = max(0, new_temp - (self.cooling+i)) if not np.isnan(new_temp) else 0.0
